(function(e) {
	"function" === typeof define && define.amd ? define([ "jquery", "datatables.net", "datatables.net-buttons" ], function(d) {
		...
		d || (d = window);
		if (!a || !a.fn.dataTable)
			a = require("datatables.net")(d, a).$;
		a.fn.dataTable.Buttons || require("datatables.net-buttons")(d, a);return e(a, d, d.document)
	...
	i.ext.buttons.print = {
		...
		text : function(c) {
			return c.i18n("buttons.print", "打印")
		},
		action : function(c, b, a, f) {
			...
			b = '<table class="' + b.table().node().className + '">';f.header && (b += "<thead>" + a(c.header, "th") + "</thead>");
			for (var b = b + "<tbody>", j = 0, i = c.body.length; j < i; j++) b += a(c.body[j], "td");
			b += "</tbody>";f.footer && c.footer && (b += "<tfoot>" + a(c.footer, "th") + "</tfoot>");
			var h = d.open("", ""),
				c = f.title;
			"function" === typeof c && (c = c());-1 !== c.indexOf("*") && (c = c.replace("*", e("title").text()));h.document.close();
			var k = "<title>" + c + "</title>";
			...
			h.document.body.innerHTML = "<h1>" + c + "</h1><div>" + f.message + "</div>" + b;f.customize && f.customize(h);
			setTimeout(function() {
