(function(g) {
	"function" === typeof define && define.amd ? define([ "jquery", "datatables.net", "datatables.net-buttons" ], function(i) {
		...
		k.fn.dataTable.Buttons || require("datatables.net-buttons")(i, k);return g(k, i, i.document)
	...
	function l(a, b, d) {
		...
		g.each(d.children, function(a, b) {
			c.appendChild(b)
		}), d.text && c.appendChild(a.createTextNode(d.text)));return c
	}
	function y(a, b) {
			...
			if (g.isPlainObject(d)) u(d);else {
				if (n) {
					...
					for (e = c.attributes.length - 1; 0 <= e; e--) {
						f = c.attributes[e].nodeName;var i = c.attributes[e].nodeValue;
						-1 !== f.indexOf(":") && (h.push({
							name : f,
							value : i
						}), c.removeAttribute(f))
				...
				c = v.serializeToString(d);n && (-1 === c.indexOf("<?xml") && (c = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + c), c = c.replace(/_dt_b_namespace_token_/g, ":"));
	...
	var j = g.fn.dataTable,
		h = {
			...
			getDOMObjectPosition : function(a) {
				...
				"" !== a.style.width && (b.width = a.style.width.replace("px", ""));"" !== a.style.height && (b.height = a.style.height.replace("px",
					""));
			...
			Client : function(a) {
				this.handlers = {};
				this.id = h.nextId++;
				this.movieId = "ZeroClipboard_TableToolsMovie_" + this.id;h.register(this.id, this);a && this.glue(a)
			}
		};
	h.Client.prototype = {
		id : 0,
		...
		glue : function(a, b) {
			...
			e.zIndex = d;"undefined" != typeof b && "" !== b && (this.div.title = b);0 !== c.width && 0 !== c.height && (this.sized = !0);this.domElement && (this.domElement.appendChild(this.div), this.div.innerHTML = this.getHTML(c.width, c.height).replace(/&/g, "&amp;"))
		},
		positionElement : function() {
			...
			b.position = "absolute";
			b.width = a.width + "px";
			b.height = a.height + "px";0 !== a.width && 0 !== a.height && (this.sized = !0, b = this.div.childNodes[0], b.width = a.width, b.height = a.height)
		},
	...
	var w = function(a,
		...
		q = function(a, b) {
			...
			d = d.replace(/[^a-zA-Z0-9_\u00A1-\uFFFF\.,\-_ !\(\)]/g, "");return b === o || !0 === b ? d + a.extension : d
		},
		z = function(a) {
			var b = "Sheet1";
		...
		s = {
			...
			init : function(a, b, d) {
				...
				var c = new h.Client;
				c.setHandCursor(!0);c.addEventListener("mouseDown", function() {
					d._fromFlash = !0;a.button(b[0]).trigger();
					d._fromFlash = !1
				});w(c, b);
				d._flash = c
			},
	...
	j.Buttons.swfPath = "//cdn.datatables.net/buttons/1.2.0/swf/flashExport.swf";j.Api.register("buttons.resize()", function() {
	...
	j.ext.buttons.copyFlash = g.extend({}, s, {
		...
		action : function(a, b, d, c) {
			c._fromFlash && (a = c._flash, d = x(b, c), c = c.customize ? c.customize(d.str, c) : d.str, a.setAction("copy"), r(a, c), b.buttons.info(b.i18n("buttons.copyTitle",
				"已复制到剪切板"), b.i18n("buttons.copySuccess", {
				_ : "已复制 %d 行到剪切板",
				1 : "已复制 1 行到剪切板"
			}, d.rows), 3E3))
		},
	...
	j.ext.buttons.csvFlash = g.extend({}, s, {
		...
		action : function(a, b, d, c) {
			a = c._flash;
			b = x(b, c);
			b = c.customize ? c.customize(b.str, c) : b.str;a.setAction("csv");a.setFileName(q(c));r(a, b)
		},
		escapeChar : '"'
	});
	j.ext.buttons.excelFlash = g.extend({}, s, {
		...
		action : function(a, b, d, c) {
			...
			g("sheets sheet", d.xl["workbook.xml"]).attr("name", z(c));c.customizeData && c.customizeData(b);c.header && (j(b.header, e), g("row c", f).attr("s", "2"));
			for (var m = 0, n = b.body.length; m < n; m++) j(b.body[m], e);
			c.footer && b.footer && (j(b.footer, e), g("row:last c", f).attr("s", "2"));
			j = l(f, "cols");g("worksheet", f).prepend(j);
			m = 0;
			...
			c.customize && c.customize(d);u(d);a.setAction("excel");a.setFileName(q(c));a.setSheetData(d);r(a, "")
		},
		extension : ".xlsx"
	});
	j.ext.buttons.pdfFlash = g.extend({}, s, {
		...
		action : function(a, b, d, c) {
			...
			a.setAction("pdf");a.setFileName(q(c));r(a, JSON.stringify({
				title : q(c, !1),
				message : c.message,
				colWidth : f.toArray(),
