(function(g) {
	"function" === typeof define && define.amd ? define([ "jquery", "datatables.net", "datatables.net-buttons" ], function(j) {
		...
		i.fn.dataTable.Buttons || require("datatables.net-buttons")(j, i);return g(i, j, j.document, q, r)
			...
			if (g.isPlainObject(c)) {
				...
				if (v) {
					...
					for (f = e.attributes.length - 1; 0 <= f; f--) {
						h = e.attributes[f].nodeName;var k = e.attributes[f].nodeValue;
						-1 !== h.indexOf(":") && (n.push({
							name : h,
							value : k
						}), e.removeAttribute(h))
				...
				e = y.serializeToString(c);v && (-1 === e.indexOf("<?xml") && (e = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
					e), e = e.replace(/_dt_b_namespace_token_/g, ":"));
				e = e.replace(/<row xmlns="" /g, "<row ").replace(/<cols xmlns="">/g, "<cols>");a.file(b, e)
			}
		})
	}
	function o(a, b, d) {
	...
	if ("undefined" !== typeof navigator && /MSIE [1-9]\./.test(navigator.userAgent))
		...
		var w = h.document.createElementNS("http://www.w3.org/1999/xhtml", "a"),
			...
			J = function(a, b, d) {
				...
				var c = this,
					...
					u;
				c.readyState = c.INIT;b || (b = "download");
				if (O) {
					f = (h.URL || h.webkitURL || h).createObjectURL(a);setTimeout(function() {
						w.href = f;
						w.download = b;
						var a = new MouseEvent("click");
						w.dispatchEvent(a);n();B(f);
						c.readyState = c.DONE
					})
				} else {
					...
					if (d === "application/octet-stream" || z)
						g = h;
					if (H) {
						A = A + a.size;H(h.TEMPORARY, A, t(function(d) {
									...
									d.getFile(b, j, t(function(b) {
										b.createWriter(t(function(d) {
											d.onwriteend = function(a) {
												...
												c.readyState = c.DONE;C(c, "writeend", a);B(b)
											};
											d.onerror = function() {
												var a = d.error;
		...
		"undefined" !== typeof navigator && navigator.msSaveOrOpenBlob ? p = function(a, b, d) {
			d || (a = I(a));return navigator.msSaveOrOpenBlob(a, b || "download")
		} : (l.abort = function() {
	...
	var x = function(a, b) {
			...
			typeof d === "function" && (d = d());d.indexOf("*") !== -1 && (d = g.trim(d.replace("*", g("title").text())));
			d = d.replace(/[^a-zA-Z0-9_\u00A1-\uFFFF\.,\-_ !\(\)]/g, "");return b === m || b === true ? d + a.extension : d
		},
		Q = function(a) {
			var b = "Sheet1";
	...
	s.ext.buttons.copyHtml5 = {
		...
		action : function(a, b, d, c) {
			...
			a = g("<span>" + b.i18n("buttons.copyKeys", "Press <i>ctrl</i> or <i>⌘</i> + <i>C</i> to copy the table data<br>to your system clipboard.<br><br>To cancel, click this message or press escape.") + "</span>").append(d);b.buttons.info(b.i18n("buttons.copyTitle", "Copy to clipboard"), a, 0);c[0].focus();c[0].select();
			var h = g(a).closest(".dt-button-info"),
				j = function() {
					h.off("click.buttons-copy");
					g(i).off(".buttons-copy");b.buttons.info(false)
				};
			h.on("click.buttons-copy", j);g(i).on("keydown.buttons-copy", function(a) {
				a.keyCode === 27 && j()
	...
	s.ext.buttons.csvHtml5 = {
		...
		action : function(a, b, d, c) {
			a = L(b, c).str;
			b = c.charset;c.customize &&
			(a = c.customize(a, c));
			if (b !== false) {
				b || (b = i.characterSet || i.charset);b && (b = ";charset=" + b)
			} else
				b = "";
			p(new Blob([ a ], {
				type : "text/csv" + b
	...
	s.ext.buttons.excelHtml5 = {
		...
		action : function(a, b, d, c) {
			...
			g("sheets sheet", a.xl["workbook.xml"]).attr("name", Q(c));c.customizeData && c.customizeData(b);
			if (c.header) {
				d(b.header, e);g("row c", f).attr("s", "2")
			}
			for (var i = 0, l = b.body.length; i < l; i++) d(b.body[i], e);
			if (c.footer && b.footer) {
				d(b.footer, e);g("row:last c", f).attr("s", "2")
			}
			d = o(f, "cols");g("worksheet", f).prepend(d);
			i = 0;
	...
	s.ext.buttons.pdfHtml5 = {
		...
		action : function(a, b, d, c) {
			...
			b = [];c.header && b.push(g.map(a.header, function(a) {
				return {
